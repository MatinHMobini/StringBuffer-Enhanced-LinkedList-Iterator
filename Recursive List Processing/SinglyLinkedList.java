...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    private int count(Node<E> p, E fromElement, E toElement, int count){
        boolean foundfrom = count > 0;

        if(p == null && foundfrom == false ){
            return 0;
        }

        if(p == null && foundfrom == true){
            return count;
        }

        boolean checkfoundfrom = p.value.equals(fromElement) || foundfrom;
        boolean found_to = p.value.equals(toElement);

        if(checkfoundfrom == true && found_to == true){

            return count + 1;
        }

        if(checkfoundfrom == false){

            return count(p.next,fromElement,toElement,0);  // count is 0 here
        }
